...
async function status(env: Env): Promise<Response> {
  ...
  ].map(key => getState(db,key)));
  const [last,chain,syncMs,rpcStatus,error,added,removed,theoryStatus,theoryBlock,theoryEnriched,missingRoot,missingAlpha,missingPrice,backfillStatus,backfillCursor,backfillProcessed,backfillError]=values;
  ...
  return json({
    ...
    theoryBackfillStatus: backfillStatus ?? 'waiting',
    theoryBackfillCursor: Number(backfillCursor ?? 0),
    theoryBackfillLastProcessed: Number(backfillProcessed ?? 0),
...
async function blocks(req: Request, env: Env): Promise<Response> {
  ...
  const segments=await blockSegments(env,netuid,from,to,q); const total=segments.reduce((s,x)=>s+x.count,0); const key=`$."${netuid}"`; const where=blockWhere(netuid,q);
  ...
  for(const segment of segments){
    ...
    items.push(...result.results as Array<Record<string,unknown>>); remaining-=take; skip=0;
  }
