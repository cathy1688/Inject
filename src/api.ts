...
async function subnetsSummary(req: Request, env: Env): Promise<Response> {
  ...
  const items=subnetRows.results.map(meta=>{
    ...
    return {
      netuid:Number(meta.netuid), name:meta.name, status:meta.status,
      actualRao:actual, theoryRao:theory, chainBuyRao:chain,
      totalEmissionRao:actual+chain,
      deviationPct: theory!=null && theory!==0 ? (actual-theory)/theory*100 : null,
      blockCount:v[3]
    };
  });
...
function blockWhere(netuid:number, q:string):{sql:string;params:unknown[]}{
  const key=`$."${netuid}"`;
  let sql='timestamp_ms BETWEEN ? AND ? AND json_extract(payload, ?) IS NOT NULL';
  const params:unknown[]=[key];
  if(q){
    const clean=q.replace(/#/g,'').replace(/,/g,'').trim();
    sql+=` AND (CAST(block_number AS TEXT) LIKE ? OR strftime('%Y-%m-%d %H:%M:%S', timestamp_ms/1000, 'unixepoch') LIKE ?)`;
...
async function blockSegments(env:Env,netuid:number,from:number,to:number,q:string):Promise<Segment[]>{
  ...
  const where=blockWhere(netuid,q);
  for(const db of blockDatabases(env)){
    const row=await db.prepare(`SELECT COUNT(*) AS n, MIN(timestamp_ms) AS min_ts, MAX(timestamp_ms) AS max_ts FROM blocks WHERE ${where.sql}`)
      .bind(from,to,...where.params).first<{n:number;min_ts:number|null;max_ts:number|null}>();
...
async function blocks(req: Request, env: Env): Promise<Response> {
  ...
  const segments=await blockSegments(env,netuid,from,to,q);
  ...
  for(const segment of segments){
    ...
    if(skip>=segment.count){skip-=segment.count;continue;}
    const take=Math.min(remaining,segment.count-skip);
    const result=await segment.db.prepare(`
      SELECT block_number,block_hash,timestamp_ms,
        CAST(json_extract(payload, ? || '[0]') AS INTEGER) AS actual_rao,
      ...
      FROM blocks
      WHERE ${where.sql}
      ORDER BY timestamp_ms ASC, block_number ASC
      LIMIT ? OFFSET ?
    `).bind(key,key,key,key,from,to,...where.params,take,skip).all();
    items.push(...result.results as Array<Record<string,unknown>>);
...
async function exportCsv(req: Request, env: Env): Promise<Response> {
  ...
  while(offset<total){
    fakeUrl.searchParams.set('offset',String(offset));
    const response=await blocks(new Request(fakeUrl.toString()),env);
    const payload=await response.json() as {items:Array<Record<string,unknown>>;total:number};
    all.push(...payload.items);total=payload.total;offset+=payload.items.length;
    if(!payload.items.length) break;
  }
  const lines=['区块高度,区块时间,链上实际注入TAO,理论计算注入TAO,Chain Buys TAO,Total Emission TAO,偏差%'];
  for(const r of all){
    const a=num(r.actual_rao),c=num(r.chain_buy_rao),t=optionalNum(r.theory_rao);const d=t&&t!==0?(a-t)/t*100:null;
    lines.push([r.block_number,new Date(num(r.timestamp_ms)).toISOString(),(a/RAO_PER_TAO).toFixed(9),t==null?'':(t/RAO_PER_TAO).toFixed(9),(c/RAO_PER_TAO).toFixed(9),((a+c)/RAO_PER_TAO).toFixed(9),d==null?'':d.toFixed(6)].join(','));
  }
...
export async function handleApi(req: Request, env: Env): Promise<Response> {
  ...
  try {
    ...
    if(url.pathname==='/api/sync' && req.method==='POST') return json(await scanToFinalized(env,24));
