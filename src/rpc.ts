...
export class SubtensorRpc {
    ...
    ws.addEventListener('message',(event:MessageEvent)=>{
      try{
        ...
        for(const msg of (Array.isArray(parsed)?parsed:[parsed])){
          ...
          const waiter=this.pending.get(msg.id);if(!waiter)continue;
          clearTimeout(waiter.timer);this.pending.delete(msg.id);
          if(msg.error)waiter.reject(new Error(`RPC ${msg.error.code}: ${msg.error.message}`));else waiter.resolve(msg.result);
        }
      }catch{}
    });
    const rejectAll=()=>{for(const [requestId,waiter] of this.pending){clearTimeout(waiter.timer);waiter.reject(new Error('Subtensor WebSocket closed'));this.pending.delete(requestId);}};
  ...
  private prepare<T>(method:string):{id:number;promise:Promise<T>}{
    ...
    const promise=new Promise<T>((resolve,reject)=>{
      const timer=setTimeout(()=>{this.pending.delete(id);reject(new Error(`RPC timeout: ${method}`));},20_000);
  ...
  async call<T>(method:string,params:unknown[]=[]):Promise<T>{
    ...
    const ws=this.ws;if(!ws)throw new Error('WebSocket not initialized');
  ...
  async queryStorage(keys:string[],atHash:string):Promise<Map<string,string|null>>{
    ...
    for(let i=0;i<keys.length;i+=chunkSize){
      const chunk=keys.slice(i,i+chunkSize);chunk.forEach(key=>map.set(key,null));
  ...
  async keysPaged(prefix:string,atHash:string,pageSize=512):Promise<string[]>{
    const all:string[]=[];let startKey:string|null=null;
    while(true){
      const page=await this.call<string[]>('state_getKeysPaged',[prefix,pageSize,startKey,atHash]);
      if(!page?.length)break;all.push(...page);if(page.length<pageSize)break;
      const next=page[page.length-1];if(next===startKey)break;startKey=next;
    }
    return all;
  }

  close():void{try{this.ws?.close(1000,'done');}catch{}this.ws=null;}
}
