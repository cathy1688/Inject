...
const DATA_VERSION='5-observed-only';
...
async function run(db:D1Database,statements:string[]):Promise<void>{await db.batch(statements.map(sql=>db.prepare(sql)));}
...
async function ensureDataVersion(env:Env):Promise<void>{
  ...
  // Intentional clean restart: discard all previous theory/catch-up-era rows and
  // begin observed-chain monitoring from the finalized block after deployment.
  await Promise.all([
  ...
  await env.META_DB.prepare('INSERT INTO sync_state(key,value,updated_at_ms) VALUES(?,?,?)').bind('data_version',DATA_VERSION,Date.now()).run();
}
...
export function ensureSchema(env:Env):Promise<void>{
  if(!ready){
    ready=(async()=>{
      ...
      await Promise.all([run(env.BLOCKS_0,BLOCK_SCHEMA),run(env.BLOCKS_1,BLOCK_SCHEMA),run(env.BLOCKS_2,BLOCK_SCHEMA),run(env.BLOCKS_3,BLOCK_SCHEMA)]);
